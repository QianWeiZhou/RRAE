...
class DRAWModel(nn.Module):
    def __init__(self, params):
        ...
        self.encoder = nn.LSTMCell(2*self.read_N*self.read_N*self.channel*self.multi + self.dec_size, self.enc_size)
    ...
    def forward(self, x):
        ...
        for t in range(self.T):
            ...
            # Equation 3.
            x_origin = x.view(self.batch_size, -1, self.B, self.A)
            x_origin = x_origin[:,:self.channel,:,:].view(self.batch_size, -1)
            x_hat_sigle = x_origin - torch.sigmoid(c_prev)
            x_hat = x_hat_sigle.clone()
            for i in range(self.multi-1):
                x_hat = torch.cat((x_hat,x_hat_sigle.clone()), dim=1)
    ...
    def read(self, x, x_hat, h_dec_prev):
        ...
        def filter_img(img, Fx, Fy, gamma):
            img = img.view(-1, self.channel*self.multi, self.B, self.A)
            Fxt = Fx.transpose(2, 3)

            # Equation 27.
            glimpse = torch.matmul(Fy, torch.matmul(img, Fxt))
            glimpse = glimpse.view(-1, self.read_N*self.read_N*self.channel*self.multi)
    ...
    def filterbank(self, gx, gy, sigma_2, delta, N, epsilon=1e-8):
        ...
        if self.channel*self.multi > 1:
            Fx = Fx.repeat(1, self.channel*self.multi, 1, 1)
            Fy = Fy.repeat(1, self.channel*self.multi, 1, 1)
